...
- `desc()` is a single generic dispatching on the type of the input, so
  vectors, factors, dates, variable pairs, tables and whole data frames
  are described through one entry point.
- Common interfaces `Association()` and `Agreement()` collect the
  respective families of coefficients behind one call.
