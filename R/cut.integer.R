...
#'
#' A [cut()] method for integer vectors. `cut.default()`
#' labels its intervals in the mathematical `"(a,b]"` notation, which
#' is right for a continuous variable but reads awkwardly for counts: for
#' integers, `"11-20"` says the same thing more plainly than
#' `"(10,20]"`.
#'
#' @details
#' The integer labels are only constructed when `labels` is
#' `NULL` *and* `breaks` is a vector of whole numbers. A
#' scalar `breaks` (a number of intervals) is passed straight to
#' [cut.default()], which computes the cut points itself, and
#' fractional break points fall back to the default interval notation as
#' well - `"34.3-66.6"` would suggest an integer range that does not
#' exist.
#'
