# Generated by using Rcpp::compileAttributes() -> do not edit by hand
# Generator token: 10BE3573-1514-4C36-9D1C-5A225CD40393

ad_stat_cpp <- function(x) {
    .Call(`_DescToolsX_ad_stat_cpp`, x)
}

ad_test_cpp <- function(x) {
    .Call(`_DescToolsX_ad_test_cpp`, x)
}

ad_prob_exact_inf_cpp <- function(a) {
    .Call(`_DescToolsX_ad_prob_exact_inf_cpp`, a)
}

ad_prob_approx_inf_cpp <- function(a) {
    .Call(`_DescToolsX_ad_prob_approx_inf_cpp`, a)
}

ad_prob_n_cpp <- function(a, n) {
    .Call(`_DescToolsX_ad_prob_n_cpp`, a, n)
}

ad_test_r_cpp <- function(x) {
    .Call(`_DescToolsX_ad_test_r_cpp`, x)
}

assoc_cpp <- function(xR, yR, conf_level = 0.95) {
    .Call(`_DescToolsX_assoc_cpp`, xR, yR, conf_level)
}

brier_boot_cpp <- function(resp, pred, R, scaled) {
    .Call(`_DescToolsX_brier_boot_cpp`, resp, pred, R, scaled)
}

condis_pairs_tab_cpp <- function(x) {
    .Call(`_DescToolsX_condis_pairs_tab_cpp`, x)
}

condis_pairs_xy_cpp <- function(xR, yR) {
    .Call(`_DescToolsX_condis_pairs_xy_cpp`, xR, yR)
}

cstat_boot_cpp <- function(yR, xR, B = 1000L, alpha = 0.05, seed = -1L) {
    .Call(`_DescToolsX_cstat_boot_cpp`, yR, xR, B, alpha, seed)
}

contcoef_jackknife_a_cpp <- function(tab, correct = FALSE) {
    .Call(`_DescToolsX_contcoef_jackknife_a_cpp`, tab, correct)
}

contcoef_table_cpp <- function(tab, correct = FALSE) {
    .Call(`_DescToolsX_contcoef_table_cpp`, tab, correct)
}

contcoef_table_boot_cpp <- function(tab, R, seed, correct = FALSE) {
    .Call(`_DescToolsX_contcoef_table_boot_cpp`, tab, R, seed, correct)
}

isoWeek_cpp <- function(x) {
    .Call(`_DescToolsX_isoWeek_cpp`, x)
}

usWeek_cpp <- function(x) {
    .Call(`_DescToolsX_usWeek_cpp`, x)
}

isoYear_cpp <- function(x) {
    .Call(`_DescToolsX_isoYear_cpp`, x)
}

isoYearweek_cpp <- function(x) {
    .Call(`_DescToolsX_isoYearweek_cpp`, x)
}

usYearweek_cpp <- function(x) {
    .Call(`_DescToolsX_usYearweek_cpp`, x)
}

usYearmonth_cpp <- function(x) {
    .Call(`_DescToolsX_usYearmonth_cpp`, x)
}

isLeapYearDate_cpp <- function(x) {
    .Call(`_DescToolsX_isLeapYearDate_cpp`, x)
}

isLeapYearInt_cpp <- function(x) {
    .Call(`_DescToolsX_isLeapYearInt_cpp`, x)
}

top_n_cpp <- function(x, n = 5L) {
    .Call(`_DescToolsX_top_n_cpp`, x, n)
}

bottom_n_cpp <- function(x, n = 5L) {
    .Call(`_DescToolsX_bottom_n_cpp`, x, n)
}

top_i_cpp <- function(v, n) {
    .Call(`_DescToolsX_top_i_cpp`, v, n)
}

bottom_i_cpp <- function(v, n) {
    .Call(`_DescToolsX_bottom_i_cpp`, v, n)
}

fast_mode_cpp <- function(x, narm = FALSE) {
    .Call(`_DescToolsX_fast_mode_cpp`, x, narm)
}

hlqest_cpp <- function(xIn) {
    .Call(`_DescToolsX_hlqest_cpp`, xIn)
}

hl2qest_cpp <- function(xIn, yIn) {
    .Call(`_DescToolsX_hl2qest_cpp`, xIn, yIn)
}

hoeffdingD_cpp <- function(permIn) {
    .Call(`_DescToolsX_hoeffdingD_cpp`, permIn)
}

n_pow_sum_cpp <- function(x) {
    .Call(`_DescToolsX_n_pow_sum_cpp`, x)
}

skew_cpp <- function(x, mean) {
    .Call(`_DescToolsX_skew_cpp`, x, mean)
}

skew_weighted_cpp <- function(x, mean, w) {
    .Call(`_DescToolsX_skew_weighted_cpp`, x, mean, w)
}

kurt_cpp <- function(x, mean) {
    .Call(`_DescToolsX_kurt_cpp`, x, mean)
}

kurt_weighted_cpp <- function(x, mean, w) {
    .Call(`_DescToolsX_kurt_weighted_cpp`, x, mean, w)
}

tbrm_cpp <- function(x, C) {
    .Call(`_DescToolsX_tbrm_cpp`, x, C)
}

tbrm_boot_cpp <- function(x, R = 999L, alpha = 0.05, constant = 9.0, seed = -1L, method = "perc") {
    .Call(`_DescToolsX_tbrm_boot_cpp`, x, R, alpha, constant, seed, method)
}

himed_weighted_cpp <- function(x, w) {
    .Call(`_DescToolsX_himed_weighted_cpp`, x, w)
}

himed_int_weighted_cpp <- function(x, iw) {
    .Call(`_DescToolsX_himed_int_weighted_cpp`, x, iw)
}

