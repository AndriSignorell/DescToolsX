...
- breaks:
  ...
  either a vector of two or more cut points or a single number (greater
  than or equal to 2) giving the number of intervals into which x is to
  be cut. For dates, a character interval specification accepted by
  [`cut.Date()`](https://rdrr.io/r/base/cut.POSIXt.html), such as
  `"days"`, `"weeks"`, `"months"`, `"quarters"`, or `"years"`, can also
  be used. For numeric variables, the default is taken from
  [`hist()`](https://rdrr.io/r/graphics/hist.html); for dates,
  calendar-aligned cut points are generated by
  [`pretty()`](https://rdrr.io/r/base/pretty.html). If set to `FALSE`, a
  numeric or date `x` will not be classed at all, but tabulated directly
  by its distinct values. This is useful for discrete numeric variables
  such as counts or scores. The argument is ignored if x is neither
  numeric nor a date.
...
- ord:
  ...
  numeric or a date and classed (`breaks` not `FALSE`).
...
#>         level  freq   perc  cumfreq  cumperc
#> 1  2025-01-01    31  31.0%       31    31.0%
#> 2  2025-02-01    28  28.0%       59    59.0%
#> 3  2025-03-01    31  31.0%       90    90.0%
#> 4  2025-04-01    10  10.0%      100   100.0%
```
