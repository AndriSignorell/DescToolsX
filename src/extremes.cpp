...
template <int RTYPE>
Vector<RTYPE> modeImpl(Vector<RTYPE> x, bool narm) {
  using storage_type = typename Rcpp::traits::storage_type<RTYPE>::type;

  if (narm)
    x = x[!is_na(x)];

  int maxFreq = 1;
  std::vector<storage_type> modes;
  std::unordered_map<storage_type, int> frequencies;
  frequencies.reserve(x.size());
    
  for (std::size_t i = 0, len = x.size(); i < len; ++i) {
    auto it = frequencies.find(x[i]);
    ...
    if (it != frequencies.end()) {
      ...
      frequencies.insert({x[i], 1});
    }
  }
    
  Vector<RTYPE> result(modes.size());
  std::copy(modes.cbegin(), modes.cend(), result.begin());

  // Preserve factor attributes without assigning through Rcpp proxies.
  ...
  result.attr("freq") = maxFreq;
  return result;
}


template <>
Vector<CPLXSXP> modeImpl(Vector<CPLXSXP>, bool) {
  stop("Complex vectors are not supported");
}


// [[Rcpp::export(name="fast_mode_cpp", rng=false)]]
SEXP fast_mode_cpp(SEXP x, bool narm = false) {
  RCPP_RETURN_VECTOR(modeImpl, x, narm);
}
